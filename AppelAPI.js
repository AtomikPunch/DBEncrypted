...
async function fetchWeatherData() {
    try {
        // Appel API
        const response = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=peshawar&appid=${process.env.API_KEY}`);
        ...
        db.run(`INSERT INTO Weather (Temperature, Humidity) VALUES (?, ?)`, [temperature, humidity], function(err) {
            if (err) {
                console.error('Database Insertion Error:', err.message);
                return;
            }
            console.log(`Weather data inserted for - Temperature: ${temperature}°C, Humidity: ${humidity}%`);
        });
        ...
        console.error('An error occurred while fetching or processing weather data:', error.message);
